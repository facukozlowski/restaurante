...
const mostrarPlatos = (ensaladas, pastas, pizzas, postres, todos) => {
    btnEnsaladas.addEventListener('click', () => {
        ...
        ensaladas.forEach(ensaladas => contenedorPlatos.appendChild(ensaladas));
    });
    btnPastas.addEventListener('click', () => {
        limpiarHtml(contenedorPlatos);
        pastas.forEach(pastas => contenedorPlatos.appendChild(pastas));
    });
    btnPizzas.addEventListener('click', () => {
        limpiarHtml(contenedorPlatos);
        pizzas.forEach(pizzas => contenedorPlatos.appendChild(pizzas));
    });
    btnPostres.addEventListener('click', () => {
        limpiarHtml(contenedorPlatos);
        postres.forEach(postres => contenedorPlatos.appendChild(postres));
    });
